...
%
\item If there is a pair of distinct nodes $[E']$ and $[F']$ such that $[E']$ and $[F']$ belong to the same connected component of $ \Ee^{atm}_{\Gg'}([E']) \cup \Ee^{atm}_{\Gg'}([F'])$,  all the arcs from $ \Ee^{atm}_{\Gg'}([E']) \cup \Ee^{atm}_{\Gg'}([F'])$  are predicate-labeled, and for each $e \in \Ee^{atm}_{\Gg'}([E']) \cup \Ee^{atm}_{\Gg'}([F'])$, it holds that $E', F'$ belong to the same node in $\persistsimp(\Gg', \atom(e))$, then merge $[E']$ and $[F']$. 
...
%
\item  For $\varphi_2$,  $\persistsimp(\Gg'_2, a_1)$ (resp. $\persistsimp(\Gg'_2, a_2)$) is obtained from $\Gg'_2$ by contracting $a_2$ and $a_3$ (resp. $a_1$ and $a_3$), then $E_1 = E_4 = E_3 = E_2$ in  $\persistsimp(\Gg'_2, a_1)$ and $E_1=E_4 = E_3 = E_5 = E_2$ in $\persistsimp(\Gg'_2,a_2)$. Therefore, $\varphi_2 \models E_1 = E_2 \wedge \ltrue$.
%
