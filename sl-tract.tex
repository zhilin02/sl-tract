...
\begin{proof}
\ \\
\noindent ``{\it Only if}'' direction: Easy.
...
select a maximal path in $\simp(\Gg)$, materialize one arc, simplify the graph, repeat until all the arcs in the path are materialized. then select another uncovered maximal path, until all the arcs are covered.

correctness: in each simplification, impossible to get $\bot$.
