...
%
\item for each pair $(E,F)$ such that $\Sigma$ contains a \emph{points-to atom} $\anatom_i=E \mapsto (\fnext,F)$, there is a \emph{directed} arc $e$ from $[E]$ to $[F]$ --- this arc is said to be \emph{field-labeled} and we write $\Ll_{\varphi}(e)= \fnext$;
%\tl{$f_0(\rho')$ here has nothing to do with function? maybe write $f_0[\rho']$}

%
\item and for each pair $(E,F)$ such that $\Sigma$ contains a \emph{predicate atom} $\anatom_i=\lseg(E;F)$, there is a \emph{directed} arc $e$ from $[E]$ to $[F]$  --- this arc is said to be \emph{predicate-labeled}  and we write $\Ll_{\varphi}(e)= \lseg$.
\end{itemize}
\end{itemize}
\end{definition}

From the construction, each directed arc $e$ corresponds to a unique atom $\anatom_i$ in $\Sigma$. Let $i(e)$ denote the index $i$ of the atom corresponding to $e$.
...
\item for each pair $(E, F)$ such that $E \neq F$ occurs in $\Pi$, there is an \emph{undirected} arc $e$ between $[E]$ and $[F]$ --- we set $\Ll_{\varphi}(e) =\neq$,
%
\item for each pair $(E,F)$ such that $\Sigma$ contains a \emph{points-to atom} $\anatom_i=E \mapsto [(\fnext,F), (\fprev,P)]$, there are a \emph{directed} arc $e$ from $[E]$ to $[F]$ labeled by $\fnext$ and a directed arc $e'$ from $[E]$ to $[P]$ labeled by $\fprev$ --- these two arcs are said to be \emph{field-labeled} and we write $\Ll_{\varphi}(e)= (\fnext,a_i)$ and $\Ll_{\varphi}(e')= (\fprev,a_i)$ respectively, moreover, we say that $a_i$ is the atom associated with $e$ and $e'$, denoted by $\atom(e)$ and $\atom({e'})$;

%
\item and for each pair $(E,F)$ such that $\Sigma$ contains a \emph{predicate atom} $\anatom_i=\dllseg(E, P;F, L)$, there are an \emph{directed} arc $e$ from $[E]$ to $[F]$ labeled by $\lseg^\fnext$ and a directed arc $e'$ from $[L]$ to $[P]$ labeled by $\lseg^\fprev$  --- these two arcs are said to be \emph{predicate-labeled}  and we write $\Ll_{\varphi}(e)= (\lseg^\fnext,a_i)$ and $\Ll_\varphi(e')=(\lseg^\fprev, a_i)$, moreover, we say that $\anatom_i$ is the atom associated with $e$ and $e'$, denoted by $\atom(e)$ and $\atom({e'})$. 
\end{itemize}
\end{itemize}
...
\end{definition}
Let $\Gg$ be an SLID[$\dllseg$] graph and $[E]$ be a node in $\Gg$. Then we use $\Ee_\Gg([E])$ to denote the set of directed arcs that are reachable from $[E]$ and $\Ee^{-1}_\Gg([E])$ denote the set of directed arcs from $[E']$ to $[F']$ such that $[E]$ is reachable from $[F']$. For an atom $\anatom_i$, if there are arcs associated with $a_i$ in $\Gg$, let $\Ee_\Gg(\anatom_i)$ denote the set $\{e,e'\}$ such that $e \neq e'$ and $\atom(e)=\atom(e')=\anatom_i$, otherwise, $\Ee_\Gg(\anatom_i) = \emptyset$. In addition, we use $\Ee^{atm}_{\Gg}([E])$ to denote $\bigcup \limits_{e \in \Ee_\Gg([E])} \Ee_\Gg(\atom(e))$. For an atom $\anatom_i$, 
...
Let $\Gg$ be an SLID[$\dllseg$]-graph and $\anatom$ be a predicate atom occurring in $\Gg$. Then a set of arcs in $\Gg$, say $\Ee'$, is said to be \emph{persistent with respect to $\anatom$} if one of the following constraints holds,
\begin{itemize}
...
Before presenting the algorithm, we introduce some notations. We say that ``contract a predicate-labeled arc $e$ from $[E]$ to $[F]$ in $\Gg$'', we mean that a new node $[E] \cup [F]$ is created, $[E] \cup [F]$ inherits all the (undirected and directed) arcs related to $[E]$ and $[F]$ in $\Gg$, except $e$, and the node $[E]$ and $[F]$ (as well as the arcs related to them) are removed.  We say that ``contract an atom $\anatom_i$'', we mean that the two arcs associated with $a_i$ are both contracted. When we say that ``merge two nodes $[E]$ and $[F]$ in $\Gg$'', we mean that a new node $[E] \cup [F]$ is created, $[E] \cup [F]$ inherits all the (undirected and directed) arcs related to $[E]$ and $[F]$, and the node $[E]$ and $[F]$ are removed. 
...
\item For each $i: 1 \le i \le n$, if $\anatom_i$ is a predicate atom such that $\Ee_{\Gg'}(\anatom_i) \neq \emptyset$ and $\Ee^{atm}_{\Gg'-\anatom_i}([E]) \cup \Ee^{atm}_{\Gg'-\anatom_i}([F])$ is persistent, then contract $\anatom_i$, where  $[E]$ and $[F]$ are the source nodes of the two arcs in $\Ee_{\Gg'}(\anatom_i)$.
\end{enumerate}
Return $\Gg'$.
\medskip

We say that a graph $\Gg'$ is \emph{persistence-irredundant} if $\persistsimp(\Gg')=\Gg'$.

%We state a property of $\persistsimp(\Gg')$.
\hide{
\begin{proposition}
Suppose $\Gg'$ is persistence-irredundant, $e$ is an arc from $[E']$ to $[F']$ in $\Gg'$ such that $\Ee_{\Gg'}([F'])$ is persistent, $e \not \in \Ee_{\Gg'}([F'])$, $e'$ is another arc out of $[E']$, and $\atom(e) \neq \atom(e')$. Let $e'' \neq e'$ be an arc such that  $\atom(e'')=\atom(e')$. Then $e'' \in \Ee_{\Gg'}([F'])$.
\end{proposition}

\begin{proof}
To the contrary, suppose $e'' \not\in \Ee_{\persistsimp(\Gg')}([F'])$. 

Since $e\not \in \Ee_{\persistsimp(\Gg')}([F'])$, we know that $e', e'' \not \in \Ee_{\persistsimp(\Gg')}([F'])$. Therefore, each arc from $\Ee_{\persistsimp(\Gg')}([F'])$ belongs to $\persistsimp(\Gg')-\atom(e')$. This implies that $\Ee_{\persistsimp(\Gg')-\atom(e')}([E'])$ is persistent and $\atom(e')$ can be contracted in $\persistsimp(\Gg')$, a contradiction to the fact that no more contractions are possible in $\persistsimp(\Gg')$. \qed
\end{proof}
}
...
\noindent
{\bf Procedure $\locinconsist(\Gg', \anatom_i)$}.\\
Input: A graph $\Gg'$  and a predicate atom $\anatom_i$.\\
%
If there is a node $[E]$ satisfying one of the following conditions, then return $true$.
\begin{itemize}
\item there is an undirected arc from $[E]$ to itself,
%
\item there are two arcs out of $[E]$ such that they are associated with two different atoms and each of them is either an arc associated with $\anatom_i$ or a field-labeled arc.
\end{itemize}
Return $false$.
...
\noindent
{\bf Procedure $\persistsimp(\Gg', \anatom_i)$}.\\
Input: A graph $\Gg'$ and a predicate atom $\anatom_i$.

\noindent Repeat the following procedure until no more reductions can be done: 
\begin{enumerate}
\item If $\locinconsist(\Gg', \anatom_i) = true$, then return $\bot$.

\item For each $j: 1 \le j \le n$ such that $j \neq i$, if $\anatom_j$ is a predicate atom such that $\Ee_{\Gg'}(\anatom_j) \neq \emptyset$  and $\Ee^{atm}_{\Gg'-a_j}([E]) \cup \Ee^{atm}_{\Gg'-\anatom_j}([F])$ is persistent with respect to $\anatom_i$, then contract $\anatom_j$, where  $[E]$ and $[F]$ are the source nodes of the two arcs in $\Ee_{\Gg'}(\anatom_j)$.
\end{enumerate}
Return $\Gg'$.

\medskip

Let $\Gg'$ be a persistence-irredundant graph, $\anatom_i$ be a predicate atom such that $\Ee_{\Gg'}(a_i) \neq \emptyset$,  and $[E_1]$ and $[E_2]$ be the source nodes of the two arcs in $\Ee_{\Gg'}(\anatom_i)$.  Then an atom $a_j$ with $j \neq i$ is said to be \emph{contractable with respect to $\anatom_i$} if  there are $r=1,2$ and a path $\pi$ from some ancestor of $[E_r]$, say $[E']$,  to $[F']$, such that the last arc of $\pi$ is an arc associated with $\anatom_j$, and $[E_r]$ is reachable from $[E']$ in $\Gg'-\{\atom(e) \mid e \in \pi\}$.

\begin{proposition}\label{prop-simp-atom}
Let $\Gg'$ be a persistence-irredundant graph, $\anatom_i$ be a predicate atom such that $\Ee_{\Gg'}(\anatom_i) \neq \emptyset$. If $\persistsimp(\Gg', \anatom_i) \neq \bot$, then $\persistsimp(\Gg', \anatom_i)$ is obtained from $\Gg'$ by contracting all the atoms that are contractable with respect to $\anatom_i$.
\end{proposition}

\begin{proof}
Let $[E_1]$ and $[E_2]$ be the source nodes of the two arcs in $\Ee_{\Gg'}(\anatom_i)$.

At first, we observe that if an atom $\anatom_j$ is contractable with respect to $\anatom_i$, then $\anatom_j$ will be contracted in $\persistsimp(\Gg',\anatom_i)$. 

Let $\Gg''$ be the graph obtained from $\Gg'$ by contracting all the atoms that are contractable with respect to $a_i$. We claim that there are no predicate atoms $\anatom_j$ such that $\Ee_{\Gg''}(\anatom_j) \neq \emptyset$  and $\Ee^{atm}_{\Gg''-a_j}([E]) \cup \Ee^{atm}_{\Gg''-\anatom_j}([F])$ is persistent with respect to $\anatom_i$,  where  $[E]$ and $[F]$ are the source nodes of the two arcs in $\Ee_{\Gg''}(\anatom_j)$.  To the contrary, suppose that such an atom $\anatom_j$ exists. Let $e_1,e_2$ be the two arcs associated with $\anatom_j$ in $\Gg''$, out of $[E]$ and $[F]$ respectively. Since $\Gg'$ is persistence-irredundant, we can infer that $\Gg''$ is persistence-irredundant as well. From the fact that $\Ee^{atm}_{\Gg''-a_j}([E]) \cup \Ee^{atm}_{\Gg''-\anatom_j}([F])$ is persistent with respect to $\anatom_i$,  we deduce that an arc associated with $\anatom_i$, say $e'$, is reachable from $[E]$ or $[F]$ in $\Gg''-\anatom_j$. Without loss of generality, suppose that $e'$ is reachable from $[E]$ in $\Gg''-\anatom_j$. Let $\pi=e_1$. Then $[E]$ is an ancestor of the source node of $e'$, the last arc of $\pi$ is an an arc associated with $\anatom_j$, and the source node of $e'$ is reachable from $[E]$ in $\Gg'-\{\atom(e'') \mid e'' \in \pi\}$. Therefore, we deduce that $\anatom_j$ is contractable with respect to $\anatom_i$, a contradiction to the fact that all such atoms have been contracted in $\Gg''$.

Therefore, we conclude that $\Gg''=\persistsimp(\Gg'', \anatom_i)=\persistsimp(\Gg', \anatom_i)$.
\qed
\end{proof}
...
Input: A graph $\Gg'$.\\
%
Repeat the following procedure until no more reductions can be done.
\begin{enumerate}
\item If $\locinconsist(\Gg') = true$, then return $\bot$.
%
...
\item $[E']$ and $[F']$ belong to the same connected component of $\Gg'$,
%
\item  each arc $e \not \in \Ee_{\Gg'}([E']) \cap \Ee_{\Gg'}([F'])$ is predicate-labeled,
%
\item and for each arc $e \not \in \Ee_{\Gg'}([E']) \cap \Ee_{\Gg'}([F'])$ such that $\persistsimp(\Gg', \atom(e)) \neq \bot$,  it holds that $[E']_e =[F']_e$, where $[E']_e$ and $[F']_e$ are the nodes of  $\persistsimp(\Gg', \atom(e))$ containing $E'$ and $F'$ respectively, 
\end{itemize}
then merge $[E']$ and $[F']$. 
%
\end{enumerate}
...
\begin{itemize}
\item For $\varphi_1$,  $\persistsimp(\Gg'_1, \anatom_1)$ (resp. $\persistsimp(\Gg'_1, \anatom_2)$) is obtained from $\Gg'_1$ by contracting $\anatom_2$ (resp. $\anatom_1$), then $E_1 =E_2 =E_3$   in  $\persistsimp(\Gg'_1, \anatom_1)$ and $E_1= E_2= E_4$ in $\persistsimp(\Gg'_1, \anatom_2)$. Therefore, $\varphi_1 \models E_1 = E_2 \wedge \ltrue$.
%
\item  For $\varphi_2$,  $\persistsimp(\Gg'_2, \anatom_1)$ (resp. $\persistsimp(\Gg'_2, \anatom_2)$) is obtained from $\Gg'_2$ by contracting $\anatom_2$ and $\anatom_3$ (resp. $\anatom_1$ and $\anatom_3$), then $E_1 = E_4 = E_3 = E_2$ in  $\persistsimp(\Gg'_2, \anatom_1)$ and $\persistsimp(\Gg'_2, \anatom_2)$. Therefore, $\varphi_2 \models E_1 = E_2 \wedge \ltrue$.
%
\item  For $\varphi_3$,  $\persistsimp(\Gg'_3, \anatom_1)$ (resp. $\persistsimp(\Gg'_3, \anatom_2)$, $\persistsimp(\Gg'_3, \anatom_3)$) is obtained from $\Gg'_3$ by contracting $\anatom_2$ and $\anatom_3$ (resp. $\anatom_1$ and $\anatom_3$, $\anatom_1$ and $\anatom_2$), then $E_1 = E_4 = E_2$ in  $\persistsimp(\Gg'_3, \anatom_1)$, $E_1=E_5 = E_2$ in $\persistsimp(\Gg'_3, \anatom_2)$, and $E_1=E_3 = E_2$ in $\persistsimp(\Gg'_3, \anatom_3)$. In addition, $\persistsimp(\Gg'_3, \anatom_4)$ is obtained from $\Gg'_3$ by  contracting $\anatom_1,\anatom_2,\anatom_3$. So $E_1 = E_3 = E_4 = E_5 = E_2$ in $\persistsimp(\Gg'_3, \anatom_4)$.  Therefore, $\varphi_3 \models E_1 = E_2 \wedge \ltrue$.
%
\item  For $\varphi_4$,  both $\persistsimp(\Gg'_4, \anatom_1)$ and $\persistsimp(\Gg'_4, \anatom_2)$ are obtained from $\Gg'_4$ by contracting $a_3$ and $a_4$, then $E_1 = E_5 = E_2$ in  $\persistsimp(\Gg'_4, \anatom_1)$ and $\persistsimp(\Gg'_4, \anatom_2)$. In addition, both $\persistsimp(\Gg'_4, \anatom_3)$ and $\persistsimp(\Gg'_4, \anatom_4)$ are obtained from $\Gg'_4$ by  contracting $\anatom_1,\anatom_2$. So $E_1 = E_3 = E_4 = E_2$ in $\persistsimp(\Gg'_4, \anatom_3)$ and $\persistsimp(\Gg'_4, \anatom_4)$.  Therefore, $\varphi_4 \models E_1 = E_2 \wedge \ltrue$.
%
\item  For $\varphi_5$,  $\persistsimp(\Gg'_5, \anatom_1)$ is obtained from $\Gg'_5$ by contracting $a_2, a_3, a_4$, so $E_3 = E_2 = E_4=E_1$ in  $\persistsimp(\Gg'_5, \anatom_1)$, $\persistsimp(\Gg'_5, \anatom_2)$ is obtained from $\Gg'_5$ by contracting $a_1, a_3, a_4$, so $E_3 = E_1 = E_4=E_2$  in  $\persistsimp(\Gg'_5, \anatom_2)$, $\persistsimp(\Gg'_5, \anatom_3)$ is obtained from $\Gg'_5$ by contracting $a_1, a_2, a_4$, so $E_3= E_1 = E_2$, $\persistsimp(\Gg'_5, \anatom_4)$ is obtained from $\Gg'_5$ by contracting $a_1, a_2, a_3, a_5$, so $E_3=E_1 = E_2$. Therefore, $\varphi_4 \models E_1 = E_2 \wedge \ltrue$.
\end{itemize}
\end{example}
...
\begin{enumerate}
\item Select a predicate atom $\anatom$ such that the two arcs of $\Gg'$ associated with $\anatom$ are the forward arc $e_1$ and the backward arc $e_2$, whose source and destination nodes are $E'_1, F'_1$ and $E'_2, F'_2$ respectively. Let $\Gg':=\persistsimp(\Gg',\anatom)$.

\item If $\Gg' \neq \bot$, then modify $\Gg'$ by applying the following replacements. 
\begin{itemize}
...
To guarantee that the above procedure indeed constructs a model of $\varphi$ in the end, it is necessary to show that the condition $\Gg' \neq \bot$ always holds in the second step.

We first observe that the graph $\Gg'$ before the execution of the first step of the above procedure is persistence-irredundant.

To the contrary, suppose that during some execution of the loop, it happens that $\Gg' \neq \bot$ but $\persistsimp(\Gg', \anatom) = \bot$, where $\Gg'$ is the graph before executing the first step. From the description of $\persistsimp(\cdot,\cdot)$, we know that this can only happen if  one of the following situation holds,
\begin{itemize}
\item there are two distinct nodes  $[E']$ and $[F']$ of $\Gg'$ such that $E',F'$ belong to the same node in $\persistsimp(\Gg', \anatom)$, and there is an undirected arc between $[E']$ and $[F']$ in $\Gg'$,

\item in $\persistsimp(\Gg', \anatom)$, there are a node $[E]$ and two arcs out of $[E]$ such that the two arcs are associated with two different atoms and each of them is either an arc associated with $\anatom$ or a field-labeled arc.
\end{itemize}

For the former situation, from Proposition~\ref{prop-simp-atom}, we deduce that there are an ancestor of $[E'_1]$ or $[E'_2]$, say $[E'_0]$, and two (possibly empty) paths $\pi_1, \pi_2$ from $[E'_0]$ to $[E']$ and $[F']$ respectively,  such that $[E'_1]$ or $[E'_2]$ is reachable from $[E'_0]$ in both $\Gg'-\{\atom(e'') \mid e'' \in \pi_1\}$ and $\Gg'-\{\atom(e'') \mid e'' \in \pi_2\}$. From this, we claim that $[E'_1]$ or $[E'_2]$ is reachable from $[E'_0]$ in $\Gg'-\{\atom(e'') \mid e'' \in \pi_1 \cup \pi_2\}$. Because otherwise, 

%Since $[E']$ and $[F']$ are two distinct nodes in $\Gg'$, we know that at least one of $\pi_1$ and $\pi_2$ is nonempty. 
Then there is an arc $e'_1$ from $[E'_0]$ to some node $[F''_1]$  in  $\Gg'-\{\atom(e'') \mid e'' \in \pi_1 \cup \pi_2\}$ such that $[E'_1]$ or $[E'_2]$ is reachable from $[F''_1]$ in $\Gg'-\{\atom(e'') \mid e'' \in \pi_1 \cup \pi_2\}$. 

This implies that $\Ee_{\Gg' - \atom(e'_1)}([E'_0])$ is persistent and $e'_1$ can be contracted in $\Gg'$,  which contradicts to the fact that $\Gg'$ is persistence-irredundant.

\zhilin{The arguments for the former situation should be cleaned}

For the latter situation, we exemplify the arguments for the case that in $\persistsimp(\Gg', \anatom)$, there are two field-labeled arcs out of $[E]$ and the two arcs belong to two different atoms. From Proposition~\ref{prop-simp-atom}, we deduce that there are two field-labeled arcs in $\Gg'$, whose source and destination nodes are $[E''_1], [F''_1]$ and $[E''_2], [F''_2]$ respectively, an ancestor of $[E'_1]$ or $[E'_2]$, say $[E'_0]$, and two (possibly empty) paths $\pi_1, \pi_2$ from $[E'_0]$ to $[E''_1]$ and $[E''_2]$ respectively,  such that $[E'_1]$ or $[E'_2]$ is reachable from $[E'_0]$ in both $\Gg'-\{\atom(e'') \mid e'' \in \pi_1\}$ and $\Gg'-\{\atom(e'') \mid e'' \in \pi_2\}$. 
Since $\Gg'$ is persistence-irredundant, we know that $\pi_1$ or $\pi_2$ is nonempty. 
Without loss of generality, suppose that $\pi_1$ is nonempty. 
Then there is an arc $e'_1$ from $[E'_0]$ to some node $[F'''_1]$  in  $\Gg'-\{\atom(e'') \mid e'' \in \pi_1\}$ such that $[E'_1]$ or $[E'_2]$ is reachable from $[F'''_1]$ in $\Gg'-\{\atom(e'') \mid e'' \in \pi_1\}$. This implies that $\Ee_{\Gg' - \atom(e'_1)}([E'_0])$ is persistent and $e'_1$ can be contracted in $\Gg'$,  which contradicts to the fact that $\Gg'$ is persistence-irredundant.
\qed
\end{proof}
