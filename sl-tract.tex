...
Let $\varphi = \Pi \wedge \Sigma$ be a SLID[$\dllseg$] formula. Our goal is to show that the satisfiability of $\varphi$ can be decided in polynomial time.

Let $\varphi = \Pi \wedge \Sigma$ be an SLID$[\dllseg]$ formula where $\Sigma \equiv a_1 \sep \dots \sep a_n$. We use $\sim_\Pi$ to denote the equivalence relation defined as follows: For $E,F \in \lvars(\varphi)$, $E \sim_\Pi F$ iff $\Pi \models E = F$. Note that the equivalence relation $\sim_\Pi$ can be computed from $\Pi$ in polynomial time. Moreover, we use $[E]_\Pi$ to denote the equivalence class of $\sim_\Pi$ containing $E$. We also abbreviate $[E]_\Pi$ as $[E]$ if $\Pi$ is clear from the context.

The challenge here is to find a proper definition of  ``allocated'' nodes.
...
%
\item and for each pair $(E,F)$ such that $\Sigma$ contains a \emph{predicate atom} $a_i=\dllseg(E, P;F, L)$, there is a \emph{directed} arc $e$ from $[E]$ to $[F]$ labeled by $(\dllseg, P, L)$  --- this arc is said to be \emph{predicate-labeled}  and we write $\Ll_{\varphi}(e)= (\dllseg, P,L)$, \fu{denoted by $\dllseg(P,L)$.}
\end{itemize}
\end{itemize}
\end{definition}
...
\begin{definition}[Potentially reachable]
...
\begin{example}
example here for the concept of potentially reachable.
\end{example}
...
%Satisfiability: Suppose the graph in normal form is not empty, then we

%can check the satisfiability as follows:
...
\end{itemize}
%
\item If there are a node $[E']$ and a pair of variables $E, F$ such that $[E] \neq [F]$, there are at least two arcs out of $[E']$, and for each arc $e$ out of $[E']$, it holds that $\big [\bigwedge \limits_{e' \in \Ee([E']) \setminus \Ee_{-e}([E'])} \phi_{e'} \big] \models E=F$, then merge $[E]$ and $[F]$.
%
\item If there are two distinct nodes $[E]$ and $[F]$ and two node-disjoint simple paths from $[E]$ to $[F]$ (except the endpoints $[E],[F]$), then merge $[E]$ and $[F]$.
\end{enumerate}
Note that $\Ee([E']) \setminus \Ee_{-e}([E'])$ denote the set of arcs such that they are reachable from $[E']$, in addition, they become unreachable from $[E']$ if $e$ is removed, in other words, every path from $[E']$ to the arc go through $e$.

\smallskip

\noindent {\bf Step III}. Let $\Gg''' :=\Gg''$. If $\Gg''' \neq \bot$, then repeat the following procedure until no more reductions can be done.
\begin{enumerate}
\item If there is a node $[E]$ satisfying one of the following conditions, then let $\Gg''':=\bot$ and exit the loop,
\begin{itemize}
\item there is an undirected arc from $[E]$ to itself,
...
\end{itemize}
%
\item If there is an arc $e$ from $[E]$ to $[F]$ such that $\Ee_{ptn,-e}([E])$ is persistent, then contract $e$.
\end{enumerate}



\begin{proposition} \label{prop:nfdllseg}
Let $\Gg$ be an SLID[$\dllseg$] graph and $\Gg'''$ be the graph obtained from $\Gg$ after Step I-III.  Then $\Gg''' = \bot$ iff $\Gg$ is unsatisfiable, in addition, if $\Gg'''  \neq \bot$, then $\Gg$ and $\Gg'''$ are equivalent. Moreover, every pair of SLID[$\dllseg$] graphs $\Gg'''_1,\Gg'''_2$ obtained from $\Gg$ after Step I-III are isomorphic.
\end{proposition}
...
\hide{
We repeat the following procedure to construct a normalised graph from $\Gg$. When we say that ``contract a predicate-labeled arc $e$ from $[E]$ to $[F]$'', we mean that a new node $[E] \cup [F]$ is created, $[E] \cup [F]$ inherits all the (undirected and directed) arcs related to $[E]$ and $[F]$, except $e$, and the node $[E]$ and $[F]$ (as well as the arcs into or out of them) are removed.
%each arc $e'$ from $[E]$ to $[F]$ or $[F]$ to $[E]$ is replaced by an arc from $[E] \cup [F]$ to itself, each arc from $[E']$ to $[E]$ (resp. from $[E]$ to $[E']$) is replaced with an arc from $[E']$ to $[E] \cup [F]$ (resp. from $[E] \cup [F]$ to $[E']$), and each arc from $[F']$ to $[F]$ (resp. from $[F]$ to $[F']$) is replaced with an arc from $[F']$ to  $[E] \cup [F]$ (resp. an arc from $[E] \cup [F]$ to $[F']$).
%
Moreover, $[E] \cup [F]$ is set to be allocated iff either $[E]$ or $[F]$ is allocated.
When we say that ``merge two nodes $[E]$ and $[F]$'', we mean that a new node $[E] \cup [F]$ is created, $[E] \cup [F]$ inherits all the (undirected and directed) arcs related to $[E]$ and $[F]$, and the node $[E]$ and $[F]$ are removed. Moreover, $[E] \cup [F]$ is set to be allocated iff either $[E]$ or $[F]$ is allocated.
\begin{description}
\item[Step I] If there is a node $[E]$ satisfying one of the following conditions, then return $\bot$,
%\tl{remark: corresponds to \cite{CHO+11}, (i)}
\begin{itemize}
\item there is an undirected arc from $[E]$ to itself,
%
\item there are two field-labeled arcs out of $[E]$.
%
%\item there are two arcs from $[E]$ to $[F_1]$ and $[F_2]$ respectively such that  $[F_1] \neq [F_2]$, the arc from $[E]$ to $[F_1]$ is field-labeled, and $[F_2]$ is allocated,

%\item there are two arcs from $[E]$ to $[F_1]$ and $[F_2]$ respectively such that $[F_1] \neq [F_2]$, and $[F_1], [F_2]$ are allocated,

%\item $[E]$ is supported by two distinct successors $[F_1], [F_2]$ and it has another allocated successor $[F_3]$,
\end{itemize}
%
\item[Step II (for C1)] If there are two distinct nodes $[E]$ and $[F]$ and two node-disjoint simple paths from $[E]$ to $[F]$ (except the endpoints $[E],[F]$), then merge $[E]$ and $[F]$.
%
\item[Step III (for C2)] If there is an arc $e$ from $[E]$ to $[F]$ such that $\Ee([E]) \setminus (\{e\} \cup \Ee([F]))$ is persistent, then contract $e$.
% are a node $[E]$ and an arc $e$ such that $e$ is either a field-labeled arc from $[E]$ to some node $[F]$ or a predicate-labeled arc from $[E]$ to some allocated node $[F]$, then for each (predicate-labeled) arc $e'$ from $[E]$ to $[F']$ such that $[F']  \neq [F]$,  contract $e'$.
%
%\item[Step IV (for C3)] If there is a node $[E]$ supported by $[F_1]$ and $[F_2]$, then for each (predicate-labeled) arc $e$ from $[E]$ to $[F_3]$ such that $[F_3]  \neq [F_1], [F_2]$,  contract $e$.
%
%
\item[Step IV] If none of the conditions in Step I-III is satisfied, then exit the loop and return the resulting graph.
\end{description}
It is not hard to see that if the condition in Step I is satisfied, then the resulting graph is unsatisfiable. Moreover, each of Step II-III above preserves the set of models of SLID[$\lseg$]-graphs. Therefore, we conclude that the graph $\Gg'$ constructed by the above procedure satisfies that $\Gg' = \bot$ iff $\Gg$ is unsatisfiable, in addition, $\Gg'$ and $\Gg$ are equivalent.

Note that strictly speaking, the aforementioned procedure is nondeterministic, e.g. in Step II, there may be multiple pair of nodes $[E]$ and $[F]$ satisfying the condition. In the following, we show that there is a unique (up to isomorphism) normalised graph constructed from $\Gg$, despite the nondeterminism.
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\hide{
For an arc $e$ from $[E]$ to $[F]$ labeled  by $\dllseg(P, L)$, we use $\Pi_e$ denote the pure formula $E = F \wedge P=L$.
For a node $[E]$ in $\Gg$ such that all the directed arcs potentially reachable from $[E]$ are predicate-labeled, let $\Pi_{ctr, [E]}$ denote the pure constraint obtained by contracting all these arcs, more specifically, $\Pi_{ctr, [E]}$ is the conjunction of $\Pi_e$ for all arcs $e$ that are potentially reachable from $[E]$.

\begin{definition}[Allocated nodes for $\dllseg$]
A node $[E]$ in an SLID[$\dllseg$]-graph $\Gg$ is allocated if either there is a field-labeled arc potentially reachable from $[E]$, or otherwise $\Pi_{ctr,[E]} \wedge \Pi$ is unsatisfiable.
\end{definition}

Note that the satisfiability of $\Pi_{ctr,[E]} \wedge \Pi$ can be decided in polynomial time since it contains just conjunctions of equalities and inequalities of variables.

\begin{proposition}
Let $[E]$ be a node in $\Gg_\varphi$. Then $[E]$ is allocated iff for each state $(s,h)$ such that $(s,h) \models \varphi$, it holds $s(E) \in \dom(h)$.
\end{proposition}

\begin{definition}[Normalised {SLID[$\dllseg$]} graphs]\label{def-norm-dllseg-graph}
An SLID[$\lseg$]-graph $\Gg$ is said to be normalised if the following conditions hold:
\begin{description}
\item[C1] For all pairs of distinct nodes $[E]$ and $[F]$, there are no two node-disjoint simple paths from $[E]$ to $[F]$.

\item[C2] There does not exist an arc $e$ from $[E]$ to $[F]$ such that $\Ee_{ptn}([E]) \setminus (\{e\} \cup \Ee_{ptn}([F]))$ is persistent.

%For each node $[E]$, if there is a field-labeled arc $e$ out of $[E]$ or an arc $e$ from $[E]$ to $[F]$ such that $[F]$ is allocated, then there are no other arcs out of $[E]$.
%
%\item[C3] For each allocated node $[E]$ which is supported by a pair of distinct successors $[E_1]$ and $[E_2]$, there are exactly two directed arcs out of $[E]$ (towards $[E_1]$ and $[E_2]$ respectively).
\end{description}
\end{definition}
...
\hide{
\begin{example}
Consider the formula $\varphi$ illustrated in Fig.\ref{fig-dllseg-alloc}. The node  $E_2$ should be seen as  ``allocated''. To see this, either the arc from $E_2$ to $E_3$ corresponds to an nonempty heap, or otherwise, $E_2 = E_3 \wedge E_6 = E_2$. In the latter case, since there is a points-to atom out of $E_6$, $E_2=E_6$ should be seen as allocated. From the fact that $E_1$ is allocated, we infer that the arc from $E_1$ to $E_8$ should always correspond to the empty heap. Thus, the arc from $E_1$ to $E_8$ can be contracted, that is, the arc is removed, and the pure constraint $E_1 = E_8 \wedge E_{10}=E_{11}$ is added.
\end{example}

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=0.8]{dll_alloc.pdf}
\end{center}
\caption{Graph $\Gg_\varphi$}\label{fig-dllseg-alloc}
\end{figure}

And certainly for normal forms, we have the rules:
\begin{itemize}
\item If $L$ is allocated, and $E\stackrel{(P,L)}{\rightarrow} F$, then it must be the case that $E=F$.

\item If $E\stackrel{(P,L)}{\rightarrow} F$, and $E, F$ are allocated, then $L$ cannot be allocated;

\item $\dllseg(E, P; F, L)\sep \dllseg(L, F; P, E)$, then they both are empty?! ( because if $E=F\wedge P=L$, then...; otherwise $E\neq F$, then the second cannot be empty, but $L$ is shared. )

%THE LESSON: once $\dllseg(E, P; F, L)$ is not empty, then $L$ is assigned.

%\item $\dllseg(E, P; F, L)\sep \dllseg(L, F; F', E)$, then they both are empty?!, i.e., $E=F, P=L, L=F'$.
