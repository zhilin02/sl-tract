...
\begin{example}
Suppose $\varphi:= \ltrue \wedge \dllseg(E_1, F_1; E_2, F_2) \sep \dllseg(E_2, F_2; E_3, F_3) \sep \dllseg(E_3, F_3; E_4, F_4) \sep \dllseg(E_1, F_1; E_5, F_5) \sep \dllseg(E_5, F_5; E_3, F_3)$. Let $e$ represent $\lseg^{\fnext}(E_1; E_2)$. Then $\Ee_e([E_1])=\{\lseg^{\fnext}(E_1; E_2), \lseg^{\fnext}(E_2; E_3)\}$. 
\end{example}
...
\end{itemize}
%
\item For each pair of distinct nodes $[E']$ and $[F']$, if there are a node $[E]$ and two distinct arcs $e_1,e_2$ out of $[E]$ satisfying the following constraints, then merge $[E']$ and $[F']$: all the directed arcs reachable from $[E]$ are predicate-labeled, $\Ee_{e_1}([E])$ and $\Ee_{e_2}([E])$ are atom-disjoint, in addition, $\Pi \wedge \bigwedge \limits_{e' \in \Ee_{e_1}([E])} \phi_{\atom(e')} $ and $\Pi \wedge \bigwedge \limits_{e' \in \Ee_{e_2}([E])} \phi_{\atom(e')}$ are satisfiable, $\Pi \wedge \bigwedge \limits_{e' \in \Ee_{e_1}([E])} \phi_{\atom(e')} \models E' = F'$, and $\Pi \wedge \bigwedge \limits_{e' \in \Ee_{e_2}([E])} \phi_{\atom(e')} \models E' = F'$.
%
